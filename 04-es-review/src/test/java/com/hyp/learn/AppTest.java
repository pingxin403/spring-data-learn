...
public class AppTest 
{
    ...
    public void testCreateIndex() throws UnknownHostException {
        client.admin().indices().prepareCreate("pingxin").get();
    }

    //删除索引,可以一次性删除多个
    @Test
    public void testDeleteIndex() throws UnknownHostException {
        client.admin().indices().prepareDelete("pingxin").get();
    }
    ...
    public void testCreateDocuments() {
        ...
        map.put("title", "平心");
        map.put("content", "平心其实很低调");
        map.put("hits", 100);

        //创建文档
        client.prepareIndex("pingxin", "article", "1").setSource(map).get();
    }
    ...
    public void testUpdateDocuments() {
        ...
        Map<String, Object> map = new HashMap<>();
        map.put("title", "平心2");
        map.put("content", "平心很低调2");
        map.put("hits", 200);

        //修改文档
        client.prepareUpdate("pingxin", "article", "1").setDoc(map).get();
    }
    ...
    public void testDeleteDocuments() {
        client.prepareDelete("pingxin", "article", "1").get();
    }
    ...
    public void testCreateDocumentsForQuery() {
        for (int i = 1; i <= 20; i++) {
            ...
            Map<String, Object> map = new HashMap<>();
            map.put("title", "平心" + i);
            map.put("content", "平心其实很低调" + i);
            map.put("hits", 100 + i);

            //创建文档
            client.prepareIndex("pingxin", "article", i + "").setSource(map).get();
        }
    }
    ...
    public void testFindAll() {
        ...
        SearchResponse searchResponse = client.prepareSearch("pingxin").setTypes("article")//设置index和type,允许传入多个
